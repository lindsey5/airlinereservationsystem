...
const AdminPayments = () => {
    ...
    useEffect(() => {
        const fetchAirports = async () => {
            ...
            setLoading(true)
            try{
                const response = await fetch(`/api/payment/payments?page=${state.currentPage}&&limit=50&&from=${startDate ? new Date(startDate) : ''}&&to=${endDate ? new Date(endDate) : ''}&&searchTerm=${searchTerm}`);
                if(response.ok){

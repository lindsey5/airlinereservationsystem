...
const FrontDeskForgotPassword = () => {
    ...
    const sendVerificationCode = async (e) => {
        ...
        setError('');
        setCounter(false);
        const response = await fetch('/api/front-desk/forgot-password/code',{
            method: 'POST',
            headers: {
    ...
    const resetPassword = async (e) => {
        ...
        if(confirm('Click ok to continue')){
            try{
                const response = await fetch('/api/front-desk/reset-password',{
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                    },
        ...
        <div className="user-forgot-password">
            <div className='left-container'>
                <form className='container' onSubmit={!showVerify ? 
                    ...
                    {!verified ?
                    <>
                    <h2>Front Desk Forgot Password</h2>
                    <p>{error}</p>
                    {!showVerify ? 
                    <div>
                        <p>Email Address</p>
...
export default FrontDeskForgotPassword
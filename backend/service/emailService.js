            ...
            <tbody>
              ${booking.flights[0].passengers.map(passenger => `
                <tr>
                  ...
                  <td style="padding: 10px; text-align: center;">${passenger.ticketNumber}</td>
            ...
            <tbody>
              ${line_items.map(item => `
                <tr>
                  ...
                  <td style="padding: 10px; text-align: center;">${item.amount * item.quantity}</td>
                </tr>`).join('')}
              <tr>
                <td style="padding: 10px; text-align: center;">Total</td>
                <td style="padding: 10px; text-align: center;">${line_items.reduce((total, item) => (item.amount * item.quantity) + total, 0)}</td>
              </tr>
            </tbody>
          </table>
          </div>
